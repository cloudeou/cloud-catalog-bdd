require('../MyTypeDefs');
import { ResultLevel, TestResultStatus } from '../enums';
...
export class TestResultHolder {
  ...
  static storeOutputToDataSetResult(cr, datasetindex, objToSave) {
    logger.info(`Object provided to save`);
    try {
      if (
        objToSave == null ||
        objToSave === undefined ||
        ...
        logger.debug('Nothing provided to store as OUTPUT in data-set result');
        return;
      }
      ...
      logger.debug(
        `Response stored: ${JSON.stringify(
          cr.datasets[datasetindex].response,
        )}`,
      );
    } catch (err) {
      logger.error(err);
    }
    logger.info(`Case Resulte so far: ${JSON.stringify(cr)}`);
    return cr;
  }
}
