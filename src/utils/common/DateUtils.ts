...
export class DateUtils {
  ...
  static convertISOstringToYYYYMMDDhhmmss(date: Date) {
    if (!!date) return String(date).replace(/T/, ' ').replace(/\..+/, '');
    else return '';
  }
  ...
  static tomorrow() {
    return DateUtils.formatDate(DateUtils.tomorrowDate());
  }
  ...
  static yyyymmddhhmmssms(): number {
  ...
  static timePassed(initialTime, msAfter) {
    const n = DateUtils.yyyymmddhhmmssms() * 1;
