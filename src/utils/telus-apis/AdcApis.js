...
class AdcApiUtils {
  ...
  async deActivateCustomer(cfg, customerId) {
    logger.enterMethod(
      `Using ADC web service api to get deactivate customer: ${customerId}`
    );
    // Disable TLS/SSL unauthorized verification; i.e. ignore ssl certificates
    process.env.NODE_TLS_REJECT_UNAUTHORIZED = 0;

    const api = cfg.deActivateCustomer.base + cfg.deActivateCustomer.endpoint;
    let contentType = null;
    if (!StringUtils.isEmpty(cfg.deActivateCustomer.contentType)) {
      contentType = {
        "Content-Type": cfg.deActivateCustomer.contentType,
      };
    }
    logger.debug(`api-url: ${api}
       headers: ${JSON.stringify(contentType)}`);
    ...
    let rawBody = FileSystem.readFileSync(
      cfg.deActivateCustomer.fileForBody
    ).toString();
    const keywordToReplace = "#CUSTOMERID#";
    logger.debug(
      `Replacing ${keywordToReplace} in request body with ${customerId}`
    );
    rawBody = StringUtils.replaceString(rawBody, keywordToReplace, customerId);
